...
def gen_all(interval=interval):
    ...
    global _tsStore
    if interval in _tsStore:
        # check if we've worked this out before
    ...
    for row in gentable.where('power>0'):
        if row['duid'] in unknown:
            continue
        try:
            fi = farmIds.index(row['duid'])
        except:
            if verbose:
