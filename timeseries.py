...
def stats(series, show=False, form='%5.0f' * 4):
    ...
    * minimum & maximum value
    ...
    smax = max(series)
    if show:
        print form % (mean, sdev, smin, smax)
...
def powerPDF(series, capacity, title='', bins=100,
    ...
    cdf, _, _ = plt.hist(s, bins, cumulative=1, normed=norm_prob,
                         histtype='step')
    if norm_prob:
        plt.ylim(0, 1)  # otherwise it sometimes goes up to 1.2
    plt.xlabel(xlab)
    plt.ylabel(ylab2)
    # save plot
    if save:
...
def ncorr(x, y):
    ...
    xlen = len(x)
    if xlen != len(y):
        print 'ncor WARNING: arrays of different length.'
    X = x - x.mean()
    Y = y - y.mean()
